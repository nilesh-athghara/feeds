...
class ParseClass {
    private val tag = "Parseclass"//name
    ...
    {
        Log.d(tag, "parse function called with $xmlData")
        ...
        try {
            ...
            //parse factory is used when we don't know exactly which class to use
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                ...
                val tagName = xpp.name?.toLowerCase()//we should use the safe call operator here ? because here we are trying to call to lowerCase method on a value that can be null
                when (eventType) {
            ...
            for (app in applications) {
                Log.d(tag, "*********************************")
                Log.d(tag, app.toString())//to string is the overloaded function that we created in class defination
            }
        } catch (e: Exception) {
            e.printStackTrace()
            status = false
